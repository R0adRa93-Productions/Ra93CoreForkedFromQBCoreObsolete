...
local function HasItem(items, quantity)
    ...
    for _, itemData in pairs(PlayerData.items) do
        if isTable then
            for k, v in pairs(items) do
                local itemKV = {k, v}
                if itemData and itemData.name == itemKV[kvIndex] and ((quantity and itemData.quantity >= quantity) or (not isArray and itemData.quantity >= v) or (not quantity and isArray)) then
                    count += 1
                end
            end
            ...
            if itemData and itemData.name == items and (not quantity or (itemData and quantity and itemData.quantity >= quantity)) then
                return true
            end
        end
    end
...
RegisterNetEvent('inventory:client:CraftItems', function(itemName, itemCosts, quantity, toSlot, points)
    local ped = PlayerPedId()
    SendNUIMessage({
        action = "close",
    })
    isCrafting = true
    QBCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.crafting"), (math.random(2000, 5000) * quantity), false, true, {
	    ...
	    StopAnimTask(ped, "mini@repair", "fixing_a_player", 1.0)
            TriggerServerEvent("inventory:server:CraftItems", itemName, itemCosts, quantity, toSlot, points)
...
RegisterNetEvent('inventory:client:CraftAttachment', function(itemName, itemCosts, quantity, toSlot, points)
    local ped = PlayerPedId()
    SendNUIMessage({
        action = "close",
    })
    ...
    QBCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.crafting"), (math.random(2000, 5000) * quantity), false, true, {
	    ...
	    StopAnimTask(ped, "mini@repair", "fixing_a_player", 1.0)
            TriggerServerEvent("inventory:server:CraftAttachment", itemName, itemCosts, quantity, toSlot, points)
            TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[itemName], 'add')
            isCrafting = false
	end, function() -- Cancel
	    StopAnimTask(ped, "mini@repair", "fixing_a_player", 1.0)
...
RegisterNUICallback("SetInventoryData", function(data, cb)
    TriggerServerEvent("inventory:server:SetInventoryData", data.fromInventory, data.toInventory, data.fromSlot, data.toSlot, data.fromquantity, data.toquantity)
    cb('ok')
end)
...
RegisterNUICallback("GiveItem", function(data, cb)
    ...
    if player ~= -1 and distance < 3 then
        if data.inventory == 'player' then
            ...
            TriggerServerEvent("inventory:server:GiveItem", playerId, data.item.name, data.quantity, data.item.slot)
        else
