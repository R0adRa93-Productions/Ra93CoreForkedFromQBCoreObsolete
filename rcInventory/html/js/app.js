...
$(document).on("mousedown", ".item-slot", function(event) {
    switch (event.which) {
        case 3:
            ...
            toquantity = $(this).data("item").quantity;
            if (toquantity > 1) {
                toquantity = 1;
            }
            if (CanQuickMove()) {
                ...
                if (toquantity >= 0) {
                    if (
                        ...
                        swap(fromSlot, toSlot, fromInventory, toInventory, toquantity);
                    }
                }
            } else {
                InventoryError(fromInventory, fromSlot);
        ...
        if (ItemData.name !== undefined) {
            if (ItemData.name.split("_")[0] == "weapon") {
                if (!$("#weapon-attachments").length) {
                    $(".invOptionsList").append(
                        '<div class="invOptionItem" id="weapon-attachments"><p>ATTACHMENTS</p></div>'
                    );
...
function FormatItemInfo(itemData) {
    if (itemData != null && itemData.info != "") {
        if (itemData.name == "id_card") {
            ...
            $(".item-info-title").html("<p>" + itemData.label + "</p>");
            $(".item-info-description").html(
                "<p><strong>quantity of cash: </strong><span>$" +
                itemData.info.cash +
                "</span></p>"
            );
...
function handleDragDrop() {
    $(".item-drag").draggable({
        ...
        start: function(event, ui) {
            ...
            var itemData = $(this).data("item");
            var dragquantity = $("#itemQuantity").val();
            if (!itemData.useable) {
            ...
            if (dragquantity == 0) {
                if (itemData.price != null) {
                    $(this).find(".item-slot-quantity p").html("0 (0.0)");
                    $(".ui-draggable-dragging")
                        .find(".item-slot-quantity p")
                        .html("(" + itemData.quantity + ") $" + itemData.price);
                    ...
                    $(".ui-draggable-dragging")
                        ...
                        .html(
                            itemData.quantity +
                            " (" +
                            ((itemData.weight * itemData.quantity) / 1000).toFixed(1) +
                            ")"
                        );
                    $(".ui-draggable-dragging").find(".item-slot-key").remove();
                    if ($(this).parent().attr("data-inventory") == "hotbar") {
            ...
            } else if (dragquantity > itemData.quantity) {
                if (itemData.price != null) {
                    $(this)
                        .find(".item-slot-quantity p")
                        .html("(" + itemData.quantity + ") $" + itemData.price);
                ...
                if (itemData.price != null) {
                    ...
                    $(this)
                        ...
                        .html(
                            ...
                            " (" +
                            (
                                (itemData.weight * (itemData.quantity - dragquantity)) /
                                1000
                            ).toFixed(1) +
                            ")"
                        );
                    $(".ui-draggable-dragging")
                        ...
                        .html(
                            dragquantity +
                            " (" +
                            ((itemData.weight * dragquantity) / 1000).toFixed(1) +
                            ")"
                        );
                    $(".ui-draggable-dragging").find(".item-slot-key").remove();
                    if ($(this).parent().attr("data-inventory") == "hotbar") {
    ...
    $(".item-slot").droppable({
        ...
        drop: function(event, ui) {
            ...
            if (toquantity >= 0) {
                if (
                    updateweights(fromSlot, toSlot, fromInventory, toInventory, toquantity)
                ) {
                    swap(fromSlot, toSlot, fromInventory, toInventory, toquantity);
                }
            }
...
function updateweights($fromSlot, $toSlot, $fromInv, $toInv, $toquantity) {
    var otherinventory = otherLabel.toLowerCase();
        ...
        if ($fromInv.attr("data-inventory").split("-")[0] == "itemshop") {
            $fromInv
                ...
                .html(
                    ...
                    '" alt="' +
                    itemData.name +
                    '" /></div><div class="item-slot-quantity"><p>(' +
                    itemData.quantity +
                    ") $" +
                    itemData.price +
    ...
    if ($fromInv.attr("data-inventory") != $toInv.attr("data-inventory")) {
        ...
        if (toData == null || fromData.name == toData.name) {
            if (
                ...
                totalWeight = totalWeight - fromData.weight * $toquantity;
                totalWeightOther = totalWeightOther + fromData.weight * $toquantity;
            } else {
                totalWeight = totalWeight + fromData.weight * $toquantity;
                totalWeightOther = totalWeightOther - fromData.weight * $toquantity;
            }
            ...
            if (
                ...
                totalWeight = totalWeight - fromData.weight * $toquantity;
                totalWeight = totalWeight + toData.weight * toData.quantity;

                totalWeightOther = totalWeightOther + fromData.weight * $toquantity;
                totalWeightOther = totalWeightOther - toData.weight * toData.quantity;
            } else {
                totalWeight = totalWeight + fromData.weight * $toquantity;
                totalWeight = totalWeight - toData.weight * toData.quantity;
...
function optionSwitch(
    ...
    if ($toSlot < 6) {
        $toInv
            ...
            .html(
                ...
                fromData.quantity +
                " (" +
                ((fromData.weight * fromData.quantity) / 1000).toFixed(1) +
                ')</p></div><div class="item-slot-label"><p>' +
                fromData.label +
                "</p></div>"
            );
    ...
    if ($fromSlot < 6) {
        $fromInv
            ...
            .html(
                ...
                toData.quantity +
                " (" +
                ((toData.weight * toData.quantity) / 1000).toFixed(1) +
                ')</p></div><div class="item-slot-label"><p>' +
                toData.label +
                "</p></div>"
            );
    ...
    $.post(
        ...
        JSON.stringify({
            ...
            toquantity: toData.quantity,
        })
    );
}

function swap($fromSlot, $toSlot, $fromInv, $toInv, $toquantity) {
    fromData = $fromInv.find("[data-slot=" + $fromSlot + "]").data("item");
    toData = $toInv.find("[data-slot=" + $toSlot + "]").data("item");
    var otherinventory = otherLabel.toLowerCase();
    ...
    if (fromData !== undefined && fromData.quantity >= $toquantity) {
        if (fromData.unique && $toquantity > 1) {
            InventoryError($fromInv, $fromSlot);
            return;
        }
        ...
        if (
            ...
            newData.quantity = parseInt($toquantity) + parseInt(toData.quantity);
            ...
            if (fromData.quantity == $toquantity) {
                ...
                if ($toSlot < 6 && $toInv.attr("data-inventory") == "player") {
                    $toInv
                        ...
                        .html(
                            ...
                            newData.quantity +
                            " (" +
                            ((newData.weight * newData.quantity) / 1000).toFixed(1) +
                            ")</p></div>" +
                            ItemLabel
                        );
            ...
            } else if (fromData.quantity > $toquantity) {
                var newDataFrom = [];
                newDataFrom.name = fromData.name;
                newDataFrom.label = fromData.label;
                newDataFrom.quantity = parseInt(fromData.quantity - $toquantity);
                    ...
                    if ($fromSlot < 6 && $fromInv.attr("data-inventory") == "player") {
                        $fromInv
                            ...
                            .html(
                                ...
                                newDataFrom.quantity +
                                " (" +
                                ((newDataFrom.weight * newDataFrom.quantity) / 1000).toFixed(
                                    1
                                ) +
                                ")</p></div>" +
                                ItemLabel
            ...
            if (fromData.quantity == $toquantity) {
                ...
                if (
                    ...
                    combineslotData.toquantity = $toquantity;
                    return;
                }
                ...
                fromData.quantity > $toquantity &&
                (toData == undefined || toData == null)
            ) {
                var newDataTo = [];
                newDataTo.name = fromData.name;
                newDataTo.label = fromData.label;
                newDataTo.quantity = parseInt($toquantity);
                ...
                if ($toSlot < 6 && $toInv.attr("data-inventory") == "player") {
                    $toInv
                        ...
                        .html(
                            ...
                            newDataTo.quantity +
                            " (" +
                            ((newDataTo.weight * newDataTo.quantity) / 1000).toFixed(1) +
                            ")</p></div>" +
                            ItemLabel
                        );
                } else if ($toSlot == 41 && $toInv.attr("data-inventory") == "player") {
...
(() => {
    ...
    Inventory.Open = function(data) {
        ...
        $(".playerInventory").find(".item-slot").remove();
                    ...
                    if (item.slot < 6) {
                        ...
                        $(".playerInventory")
                            ...
                            .html(
                                ...
                                item.quantity +
                                " (" +
                                ((item.weight * item.quantity) / 1000).toFixed(1) +
                                ")</p></div>" +
                                ItemLabel
                            );
                        $(".playerInventory")
        ...
        if (
            ...
            $.each(data.other.inventory, function(i, item) {
                if (item != null) {
                    totalWeightOther += item.weight * item.quantity;
                    var ItemLabel =
                        '<div class="item-slot-label"><p>' + item.label + "</p></div>";
                    if (item.name.split("_")[0] == "weapon") {
                        if (!Inventory.IsWeaponBlocked(item.name)) {
        ...
        if (data.other != null) {
            ...
            if (
                ...
                $("#otherInvLabel").html(data.other.label);
            } else {
                $("#otherInvLabel").html(data.other.label);
                $("#otherInvWeight").html(
                    "⚖️: " +
            ...
            $("#otherInvLabel").html(Inventory.droplabel);
            $("#otherInvWeight").html(
                "⚖️: " +
                (totalWeightOther / 1000).toFixed(2) +
                " / " +
        ...
        $.each(data.maxammo, function(index, ammotype) {
            $("#" + index + "_ammo")
                .find(".ammo-box-quantity")
                .css({ height: "0%" });
        });

        if (data.Ammo !== null) {
            $.each(data.Ammo, function(i, quantity) {
                var Handler = i.split("_");
                var Type = Handler[1].toLowerCase();
                if (quantity > data.maxammo[Type]) {
                    quantity = data.maxammo[Type];
                }
                var Percentage = (quantity / data.maxammo[Type]) * 100;
                    ...
                    if (item.slot == 41) {
                        $(".z-hotbar-inventory")
                            ...
                            .html(
                                ...
                                '" alt="' +
                                item.name +
                                '" /></div><div class="z-hotbar-item-slot-quantity"><p>' +
                                item.quantity +
                                " (" +
                                ((item.weight * item.quantity) / 1000).toFixed(1) +
                                ")</p></div>" +
    ...
    window.onload = function(e) {
        window.addEventListener("message", function(event) {
            switch (event.data.action) {
                ...
                case "RobMoney":
                    $(".invOptionsList").append(
                        '<div class="invOptionItem" id="rob-money"><p>TAKE MONEY</p></div>'
                    );
